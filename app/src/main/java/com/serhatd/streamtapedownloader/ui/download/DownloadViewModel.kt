...
class DownloadViewModel @Inject constructor(private val repo: StreamtapeRepository, private val toast: ToastManager): ViewModel() {
    ...
    private fun getVideoId(url: String): String {
        val pattern1 = "(?:streamtape\\.com\\/v\\/)([\\w\\-]+)\\/.+".toRegex()
        val pattern2 = "(?:streamtape\\.to\\/e\\/)([\\w\\-]+)".toRegex()
        val pattern3 = "(?:streamtape\\.cc\\/e\\/)([\\w\\-]+)".toRegex()

        val matchResult1 = pattern1.find(url)
        val matchResult2 = pattern2.find(url)
        val matchResult3 = pattern3.find(url)

        val videoId = matchResult1?.groupValues?.getOrNull(1) ?: matchResult2?.groupValues?.getOrNull(1) ?: matchResult3?.groupValues?.getOrNull(1)
        return videoId ?: ""
    }
